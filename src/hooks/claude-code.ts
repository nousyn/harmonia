 ...
 *
 * Hooks:
 * 1. PreToolUse — boundary guard: block code edits, non-standard paths, dev commands
 ...
 *
 * Blocks:
 * - Write/Edit tools targeting code files inside project dir
 * - Write/Edit tools targeting paths outside project dir AND Harmonia data dir
...
case "$TOOL_NAME" in
  Write|Edit|MultiEdit|write|edit)
    ...
    # Allow writes to Harmonia data directory (internal doc management)
    case "$FILE_PATH" in
      "$DATA_DIR"*) exit 0 ;;
    esac
    ...
    # Block writes to paths outside both project dir and data dir
    case "$FILE_PATH" in
 ...
 *
 * On each user prompt submission, reads Harmonia state files and injects
 * context-aware reminders:
 * - Running dispatches that exceed timeout
 * - Idle phase warning
 * - Pending document reviews
 * - Session start guidance (when no recent tool calls detected)
 */
...
if [ -f "$DISPATCHES_FILE" ]; then
  # Find running dispatches and check their age
