...
- **状态**：待实现
- **现状**：`EventLogger` 当前通过 `console.log` 输出结构化 JSON 日志到 stdout，没有写入文件。
- **问题**：服务器长期运行时，控制台日志不持久，无法事后排查问题。编排事件日志应落盘到文件中。
- **实现方向**：日志写入数据目录下的文件（如 `<data_dir>/harmonia/logs/` 或项目级 `<project>/logs/`），支持按日期或大小轮转。可保留 stdout 输出作为开发/调试模式。
